...
本指南将逐步介绍如何为 Stable 上的 USDT0 编写自定义 [MPP](/en/explanation/mpp) 支付方法，并提供一个受 MPP 保护的端点。买方签署一个 [ERC-3009](/en/explanation/erc-3009) `transferWithAuthorization`，服务器通过 `mppx` 的 `verify()` 钩子对其进行验证，结算则在你控制的单独步骤中完成。

:::note
**概念：** 关于什么是 MPP 以及它与 x402 的关系，请参阅 [机器支付协议（MPP）](/en/explanation/mpp)。关于 x402 的等价实现，请参阅 [构建按次调用付费 API](/en/how-to/build-pay-per-call)。
:::
...
- 在 Stable 上有一个已注资的 USDT0 钱包。请参阅 [使用水龙头](/en/how-to/use-faucet) 或 [转移 USDT0](/en/tutorial/send-usdt0)。
...
使用 Stable 的 [Gas Waiver](/en/how-to/integrate-gas-waiver) 以 `gasPrice = 0` 提交内层交易。卖方仍然签署包装交易，但不支付 gas。需要一个 Waiver Server API 密钥。
...
关于如何在发布前构建已签名的内层交易（`gasPrice: 0`，已编码的 `transferWithAuthorization` 调用），请参阅 [Gas waiver 协议](/en/reference/gas-waiver-api)。
...
- [**MPP 概念**](/en/explanation/mpp) — 了解 MPP 如何与 x402 关联，以及其他意图是什么样的。
- [**MPP 会话**](/en/explanation/mpp-sessions) — 当按请求结算成本过高时，使用链下凭证流式传输微支付。
- [**Facilitators**](/en/reference/agentic-facilitators) — 使用 Semantic Pay 或 Heurist 作为结算目标，而不是直接提交。
