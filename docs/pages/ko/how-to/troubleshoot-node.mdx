...
title: 문제 해결 가이드
description: "일반적인 Stable 노드 연결, 동기화, 성능 문제에 대한 진단 스크립트와 해결책."
diataxis: "how-to"
---

이 종합 가이드는 Stable 노드의 일반적인 문제를 진단하고 해결하는 데 도움을 줍니다.
...
| `connection refused` | 서비스가 실행 중이 아님 | 서비스 시작 |
| `timeout waiting for tx to be included` | 네트워크 혼잡 | 가스 가격 인상 |
| `account sequence mismatch` | 논스 오류 | 현재 논스 조회 |
| `insufficient fees` | 가스 가격이 너무 낮음 | 가스 가격 인상 |
...
- 문제를 예방하려면 [모니터링 설정](/en/how-to/monitor-node)을 검토하세요
- 버전별 문제는 [업그레이드 가이드](/en/how-to/upgrade-node)를 확인하세요
