...
:::note
**概念：** 关于 Gas Waiver 是什么、为何存在以及治理授权的豁免如何运作，请参阅 [Gas 豁免](/en/explanation/gas-waiver)。关于完整的协议规范（包装交易格式、标记地址、执行语义、安全模型），请参阅 [Gas 豁免协议](/en/reference/gas-waiver-api)。
:::
...
- [**零 Gas 交易**](/en/how-to/zero-gas-transactions) — 查看以演示为核心的流程，以及如何在收据中验证零 Gas。
- [**自托管 Gas Waiver**](/en/how-to/self-hosted-gas-waiver) — 在不使用托管 API 的情况下运行你自己的豁免服务。
- [**Gas 豁免协议**](/en/reference/gas-waiver-api) — 完整的包装交易规范和治理模型。
- [**Stable SDK**](/en/explanation/sdk-overview) — 使用类型化客户端来签署用户交易，随后将其提交到 Waiver Server。
