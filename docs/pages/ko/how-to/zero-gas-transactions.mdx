...
:::note
**개념**: 래퍼 트랜잭션 메커니즘, 권한 모델, 보안 보장에 대해서는 [Gas waiver](/en/explanation/gas-waiver)와 [Gas waiver 프로토콜 레퍼런스](/en/reference/gas-waiver-api)를 참조하세요.
:::
...
전체 정책 모델과 waiver별 범위 규칙은 [Gas waiver 프로토콜](/en/reference/gas-waiver-api)을 참조하세요.

## 다음 추천 사항

- [**Waiver 서버 통합하기**](/en/how-to/integrate-gas-waiver) — 전체 API 레퍼런스, 배치 제출, 오류 코드, NDJSON 스트리밍.
- [**셀프 호스팅 Gas Waiver**](/en/how-to/self-hosted-gas-waiver) — 자체 waiver 주소를 등록하고 호스팅된 API 없이 래퍼를 브로드캐스트합니다.
- [**Gas waiver 프로토콜**](/en/reference/gas-waiver-api) — 전체 명세를 읽어보세요: 마커 라우팅, 래퍼 형식, 거버넌스 제어.
