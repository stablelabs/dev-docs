...
Three paths produce the same on-chain result, differing in who holds the waiver key. See [Choosing an integration path](/en/explanation/gas-waiver#choosing-an-integration-path).

Against the hosted Waiver Server:

1. Collect a signed `InnerTx` from the user (`gasPrice = 0`).
2. Submit the signed inner transaction to the Waiver Server API.
3. Handle the streamed results and surface transaction hashes to end users.

From a waiver address you registered yourself, `@stablechain/enterprise` builds, signs, and broadcasts the `WrapperTx` in one call. See [Relay with gas waiver](/en/how-to/relay-with-gas-waiver), or [Self-hosted gas waiver](/en/how-to/self-hosted-gas-waiver) to construct wrappers directly.
...
- [**Relay with gas waiver**](/en/how-to/relay-with-gas-waiver): Sponsor gas from your own waiver with the typed Enterprise SDK client.
