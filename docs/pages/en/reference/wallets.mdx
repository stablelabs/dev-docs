...
CDP provides non-custodial wallets for user-facing apps and backend systems. User-auth wallets use email, SMS, or social sign-in. API-key wallets let servers manage accounts while CDP secures the private keys.
...
Stable is not a CDP-managed network. Configure it as a custom network with chain ID `988` and `https://rpc.stable.xyz`, or sign with CDP and broadcast through Stable's RPC.

Coinbase Onramp does not deliver funds directly to Stable. The funding flow is:

1. Use Coinbase Onramp on a supported network.
2. Use a separate bridge or orchestration provider to move the funds to Stable as USDT0.

CDP's managed balance, transfer, swap, staking, onramp, and offramp features should not be treated as native Stable support.
