...
이 가이드는 인보이스 메타데이터에서 파생된 결정적 nonce를 사용하여 [ERC-3009](/en/explanation/erc-3009)로 온체인에서 인보이스를 정산하는 과정을 안내합니다. nonce는 각 결제를 해당 인보이스에 연결하고 이중 결제를 방지합니다.

:::note
**개념:** 인보이스 정산 모델과 전통적인 B2B 인보이스 발행과의 비교는 [인보이스 정산](/en/reference/invoices)을 참조하세요.
:::
...
- [**인보이스 정산 개념**](/en/reference/invoices) — 결정적 nonce 대사 처리 모델을 이해하세요.
- [**ERC-3009**](/en/explanation/erc-3009) — 이 흐름의 기반이 되는 서명된 인가 표준을 검토하세요.
- [**가스 없는 트랜잭션 활성화**](/en/how-to/integrate-gas-waiver) — Gas Waiver와 결합하여 정산 경로에서 가스를 제거하세요.
