...
source_sha: dcc53024598ac323cfe971fd1599f51647055726
title: 설치 가이드
description: "사전 컴파일된 릴리스를 사용하여 Linux에 Stable 노드 바이너리를 설치하는 단계별 가이드입니다."
diataxis: "how-to"
---

이 가이드는 다양한 플랫폼에 Stable 노드를 설치하고 설정하는 방법에 대한 자세한 지침을 제공합니다.

## 전제 조건

설치를 시작하기 전에 다음 사항을 확인하십시오.
- 모든 [시스템 요구 사항](/ko/reference/node-system-requirements)을 충족했습니다.
- 서버에 대한 루트 또는 sudo 액세스 권한이 있습니다.
- Linux 명령줄에 대한 기본 지식이 있습니다.

## 설치 방법

플랫폼에 맞는 사전 컴파일된 바이너리를 사용하세요. Stable은 현재 소스에서 빌드를 지원하지 않습니다.
...
# 이렇게 하면 ~/.stabled/ 에 구성 디렉터리가 생성됩니다.
```
...
`~/.stabled/config/config.toml` 파일 편집:
...
### 3단계: Cosmovisor 디렉토리 구조 설정

```bash
# cosmovisor 디렉토리 구조 생성
...
# Cosmovisor는 업그레이드 높이에서 이 바이너리로 자동 전환됩니다.
```
...
Cosmovisor를 사용하지 않으려면 표준 systemd 서비스를 만들 수 있습니다.
...
더 빠른 동기화를 위해 [스냅샷 및 동기화 가이드](/ko/how-to/use-node-snapshots)를 참조하세요.
- 아카이브 노드 스냅샷
- 가지치기된 노드 스냅샷
