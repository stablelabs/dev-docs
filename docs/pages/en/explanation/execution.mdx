...
Stable executes EVM transactions concurrently with Block-STM while preserving a deterministic final state. Precompiles connect that execution layer to Stable SDK modules.
...
## Optimistic parallel execution in v1.8.0
...
Stable v1.8.0 runs **Optimistic Parallel Execution (OPE)** through Block-STM. Transactions execute concurrently across CPU cores, while a fixed in-block order keeps the final state deterministic.
...
Stable v1.8.0 includes **Selective RecheckTx**. The application returns the block's state-change delta, and the node rechecks only transactions from affected accounts. CometBFT detects application support and falls back to a full recheck when selective recheck is unavailable.
...
- [**Network upgrades**](/en/reference/network-upgrades): Review the v1.8.0 release scope and activation details.
