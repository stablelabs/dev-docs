...
[Autobahn](/en/explanation/autobahn) 提供了一种 PBFT-on-DAG 架构，能够更自然地与 Stable 的共识层集成，同时在正常条件下提供低延迟，并在面对网络故障时实现快速恢复。Stable 团队与 Autobahn 论文的作者保持着密切的合作关系，并将利用 Autobahn 的架构来最大化 StableBFT 的性能。
...
- [**Autobahn**](/en/explanation/autobahn) — 阅读支撑 StableBFT 基于 DAG 升级路径的协议论文。
- [**执行**](/en/explanation/execution) — 了解区块如何从共识进入并行执行。
- [**最终性**](/en/explanation/finality) — 在基于 RPC 进行构建时应用 Stable 的单槽最终性。
