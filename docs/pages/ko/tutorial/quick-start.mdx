...
:::note
타입이 지정된 클라이언트를 선호하시나요? [Stable SDK](/en/explanation/sdk-overview)는 `transfer`, `bridge`, `swap` 메서드로 viem을 감싸므로 수동 ABI 및 소수점 작업을 건너뛸 수 있습니다.
:::
...
:::warning
`maxPriorityFeePerGas`는 Stable에서 무시되며 반드시 `0`으로 설정해야 합니다. base-fee-only 모델이 트랜잭션 구성을 어떻게 변경하는지는 [가스 가격 책정](/en/reference/gas-pricing-api)을 참조하세요.
:::

## 다음 단계

- [**스마트 컨트랙트 배포**](/en/tutorial/smart-contract) — Foundry 프로젝트를 스캐폴딩하고 Stable 테스트넷에 배포하세요.
- [**결제 앱 구축**](/en/how-to/build-p2p-payments) — 지갑 생성, 전송, 수신, 결제 내역 조회를 만들어보세요.
- [**AI로 개발하기**](/en/how-to/develop-with-ai) — MCP 서버와 에이전트 스킬을 AI 에디터에 연결하세요.
