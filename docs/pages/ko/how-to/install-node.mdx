...
description: "사전 컴파일된 바이너리를 사용하여 Linux에 Stable 노드를 설치하는 단계별 가이드."
---
...
- Met all [System Requirements](./system-requirements)
...
> **Note**: For current network parameters including chain ID, see [Mainnet Information](#TODO)
...
> **Note**: For current network parameters including chain ID, see [Testnet Information](#TODO)
...
For faster synchronization, see the [Snapshots & Sync Guide](./snapshots) for:
...
1. **Configure your node**: See [Configuration Guide](./configuration)
2. **Speed up sync**: Check [Snapshots & Sync](./snapshots)
3. **Monitor your node**: Set up [Monitoring](./monitoring)
