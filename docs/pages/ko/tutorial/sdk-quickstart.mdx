...
- Node.js 20 이상
- 테스트넷 USDT0가 있는 테스트 개인 키. [테스트넷 지갑에 자금 충전하기](/en/how-to/use-faucet)를 참고하세요.
...
`createStable`은 세 가지 서명 모드를 허용합니다: `account`(서버 측, 위에 표시됨), `transport`(`custom(window.ethereum)`를 통한 브라우저 지갑), 또는 `walletClient`(미리 빌드된 viem `WalletClient`). 세 가지 모두에 대해서는 [viem과 함께 SDK 사용하기](/en/how-to/sdk-with-viem)를 참고하세요.
...
- [**SDK 레퍼런스**](/en/reference/sdk) — 모든 매개변수, 반환 타입, 오류 클래스.
- [**viem과 함께 사용하기**](/en/how-to/sdk-with-viem) — 개인 키, 브라우저 지갑, 미리 빌드된 `WalletClient` 서명 간 전환.
- [**wagmi와 함께 사용하기**](/en/how-to/sdk-with-wagmi) — wagmi 훅을 사용하여 React 앱에 SDK 연결하기.
