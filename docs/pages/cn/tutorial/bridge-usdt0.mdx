...
本教程使用 OFT Mesh 路径。Sepolia 上的 OFT Adapter 锁定你的代币，LayerZero 的双 DVN 验证确认消息，然后在 Stable 上铸造 USDT0。要全面了解其工作原理，请参阅[桥接到 Stable](/en/explanation/usdt0-bridging)。

:::note
想要更少的代码行数吗？[Stable SDK](/en/explanation/sdk-overview) 暴露了 `quoteBridge` 和 `bridge`，并会为你选择路由（LayerZero 或 LI.FI）。
:::
...
- [**发送你的第一笔 USDT0**](/en/tutorial/send-usdt0) — 使用桥接得到的 USDT0 进行原生转账和 ERC-20 转账。
- [**桥接到 Stable**](/en/explanation/usdt0-bridging) — 深入了解 OFT Mesh 与 Legacy Mesh 的机制。
- [**测试网信息**](/en/reference/testnet-information) — 完整的网络参数、RPC 端点和水龙头详情。
