...
在开始安装之前，请确保你已：
- 满足所有[系统要求](/en/reference/node-system-requirements)
...
> **注意**：有关包括 chain ID 在内的当前网络参数，请参阅[主网信息](/en/reference/mainnet-information)
...
> **注意**：有关包括 chain ID 在内的当前网络参数，请参阅[测试网信息](/en/reference/testnet-information)
...
如需更快的同步，请参阅[快照与同步指南](/en/how-to/use-node-snapshots)，了解：
- 归档节点快照
- 修剪节点快照
...
1. **配置你的节点**：参阅[配置指南](/en/reference/node-configuration)
2. **加速同步**：查看[快照与同步](/en/how-to/use-node-snapshots)
3. **监控你的节点**：设置[监控](/en/how-to/monitor-node)
4. **加入社区**：在 [Discord](https://discord.gg/stablexyz) 获取支持

## 故障排除

如果你在安装过程中遇到问题：
- 查看[故障排除指南](/en/how-to/troubleshoot-node)
- 验证是否满足系统要求
- 确保端口未被防火墙阻止
- 检查磁盘空间和权限
- 查看 systemd 日志中的错误
