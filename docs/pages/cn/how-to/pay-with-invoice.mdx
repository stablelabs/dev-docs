...
本指南演示如何使用 [ERC-3009](/en/explanation/erc-3009) 在链上结算发票，其中 nonce 由发票元数据确定性派生。该 nonce 将每笔付款与其发票关联起来，并防止重复付款。

:::note
**概念：** 关于发票结算模型以及与传统 B2B 开票方式的比较，请参阅[发票结算](/en/reference/invoices)。
:::
...
- [**发票结算概念**](/en/reference/invoices) — 了解确定性 nonce 的对账模型。
- [**ERC-3009**](/en/explanation/erc-3009) — 回顾此流程背后的签名授权标准。
- [**启用免 gas 交易**](/en/how-to/integrate-gas-waiver) — 与 Gas Waiver 结合，从结算路径中消除 gas。
