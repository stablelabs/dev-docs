...
为您的平台使用预编译的二进制文件。Stable 目前不支持从源代码构建。
...
> **注意**：有关包括链 ID 在内的当前网络参数，请参阅[主网信息](/cn/reference/mainnet-information)
...
> **注意**：有关包括链 ID 在内的当前网络参数，请参阅[测试网信息](/cn/reference/testnet-information)
...
Cosmovisor 是一个进程管理器，可以自动处理链升级。这是生产节点推荐的设置。
...
如需更快的同步，请参阅[快照和同步指南](/cn/how-to/use-node-snapshots)，其中包含：
- 归档节点快照
- 裁剪节点快照
...
sudo ufw allow 26657/tcp  # RPC（仅在外部需要时）
sudo ufw enable
```
...
2. **加速同步**：查阅[快照和同步](/cn/how-to/use-node-snapshots)
