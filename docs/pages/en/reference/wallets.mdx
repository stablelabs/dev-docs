...
CDP provides non-custodial wallet infrastructure for user-facing apps and backend systems. User-auth wallets use email, SMS, or social sign-in. API-key wallets let servers manage accounts while CDP secures private keys in a trusted execution environment.

**Capabilities**

- Embedded wallets with email, SMS, or social login and no seed phrase
- API-key wallets for automated workflows, agents, and backend services
- Wallet creation, transaction signing, balances, transfers, swaps, staking, and fiat on-ramps and off-ramps
- Support for EVM-compatible networks, including Stable, and Solana
- Key export for user-controlled wallets

**Get started**: Follow the [CDP non-custodial wallet documentation](https://docs.cdp.coinbase.com/wallets/non-custodial-wallets/overview) and choose the user-auth or API-key quickstart for your application.
