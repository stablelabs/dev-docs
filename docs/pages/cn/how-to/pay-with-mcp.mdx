...
本指南展示如何将启用 x402 的 API 桥接到 [MCP](https://modelcontextprotocol.io) 工具，使 AI 客户端能够通过自然语言提示词调用并为其付费。它基于 [构建按调用付费 API](/en/how-to/build-pay-per-call) 中的服务器构建。
...
- 一个正在运行的 x402 服务器（参见 [构建按调用付费 API](/en/how-to/build-pay-per-call)）。
...
- [**构建按调用付费 API**](/en/how-to/build-pay-per-call) — 设置此 MCP 服务器所桥接的 x402 服务器。
- [**x402 概念**](/en/explanation/x402) — 了解这些支付背后的结算协议。
- [**使用 AI 开发**](/en/how-to/develop-with-ai) — 将 Stable 的文档和运行时 MCP 服务器接入同一个 AI 客户端。
