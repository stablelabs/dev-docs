...
:::note
**概念**：关于包装交易机制、授权模型和安全保证，请参阅 [Gas waiver](/en/explanation/gas-waiver) 和 [Gas waiver 协议参考](/en/reference/gas-waiver-api)。
:::
...
关于完整的策略模型和按 waiver 的范围规则，请参阅 [Gas waiver 协议](/en/reference/gas-waiver-api)。

## 推荐的后续步骤

- [**集成 Waiver Server**](/en/how-to/integrate-gas-waiver) — 完整的 API 参考、批量提交、错误码和 NDJSON 流式传输。
- [**自托管 Gas Waiver**](/en/how-to/self-hosted-gas-waiver) — 注册你自己的 waiver 地址，无需托管 API 即可广播包装交易。
- [**Gas waiver 协议**](/en/reference/gas-waiver-api) — 阅读完整规范：标记路由、包装格式、治理控制。
