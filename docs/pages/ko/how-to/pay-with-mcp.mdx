...
이 가이드는 x402가 활성화된 API를 [MCP](https://modelcontextprotocol.io) 도구에 연결하여 AI 클라이언트가 자연어 프롬프트를 통해 이를 호출하고 비용을 지불할 수 있도록 하는 방법을 보여줍니다. 이는 [호출당 결제 API 구축하기](/en/how-to/build-pay-per-call)의 서버를 기반으로 합니다.
...
- 실행 중인 x402 서버 ([호출당 결제 API 구축하기](/en/how-to/build-pay-per-call) 참고).
...
- [**호출당 결제 API 구축하기**](/en/how-to/build-pay-per-call) — 이 MCP 서버가 브리지하는 x402 서버를 설정합니다.
- [**x402 개념**](/en/explanation/x402) — 이러한 결제 뒤에 있는 정산 프로토콜을 살펴봅니다.
- [**AI로 개발하기**](/en/how-to/develop-with-ai) — Stable의 Docs 및 Runtime MCP 서버를 동일한 AI 클라이언트에 연결합니다.
