...
:::note
**概念：** 关于 Gas 豁免是什么以及为何存在，参见 [Gas 豁免](/en/explanation/gas-waiver)。关于完整的协议规范（包装交易机制、授权、策略检查、执行语义、安全模型），参见 [Gas 豁免协议](/en/reference/gas-waiver-api)。
:::

关于托管的 Waiver Server API 集成路径，参见 [启用免 gas 交易](/en/how-to/integrate-gas-waiver)。
...
- [**Gas 豁免概念**](/en/explanation/gas-waiver) — 在运行你自己的豁免之前理解其机制。
- [**Gas 豁免协议**](/en/reference/gas-waiver-api) — 参考完整的协议规范，了解 marker 路由、授权和执行语义。
- [**启用免 gas 交易**](/en/how-to/integrate-gas-waiver) — 使用托管的 Waiver Server API，而不是自托管。
